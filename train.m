...
for i =2:dimension
    ...
    for j=1:length(sph_all)
        sph_all_temp(j,:) = script_roation (sph_all(j,:),mapping(sph_all(j,:),ortho_sph_cooked(FarIndex(i-1),i),ortho_sph_cooked(CloseIndex(i-1),i),...
            Raw_sph(FarIndex(i-1),i),Raw_sph(CloseIndex(i-1),i),i-1,twopi_range),i);
    end
    sph_all = sph_all_temp;
end
sph_all_cart = nsphere2cart(sph_all);

ro1 = sph_all_cart(1:nd1,:);
ro2 = sph_all_cart(nd1+1:nd2,:);
ro3 = sph_all_cart(nd2+1:nd3,:);
ro4 = sph_all_cart(nd3+1:nd4,:);
